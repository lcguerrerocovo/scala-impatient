...
object Chapter12 {
  ...
  //     with `foldLeft`.**
  //
  //     - (Look at the Scaladoc for `foldLeft`. It’s like `reduceLeft`, except that the
  ...
  //
  //     - For example, `largest(x => 10 * x - x * x,1 to 10)` should return `25`. Don’t use a loop or
  //     recursion.
  ...
  //
  //     - For example, `largestAt(x => 10 * x - x * x, 1 to 10)` should return `5`. Don’t use a loop or
  //     recursion.
  def largestWithParameter(fun: Int => Int, inputs: Seq[Int]): Int = {
    inputs.reduceLeft((a,b) => if(fun(a) >= fun(b)) a else b)
  }

  // **7.Write a function `adjustToPair` that receives a function of type `(Int, Int) => Int` and
  ...
  //     you can’t do: `pairs.map(_ + _)`
  //
  //     - The function _ + _ takes two Int parameters, not an (Int, Int) pair.  For example:
  //         `adjustToPair(_ * _)((6, 7)) is 42.`
  //     - Then use this function in conjunction with map to compute the sums of the elements in
  //     pairs.
  ...
  //
  //     - `corresponds(Array("cookie","sugar","tea"),Array(6,5,3),_.length.equals(_))` doesn't
  //     work because types can't be inferred by compiler throwing this error:
  //     `missing parameter type for expanded function ((x$1: <error>, x$2: <error>) =>
  //     x$1.length.equals(x$2))`
  //
  //     - `corresponds(Array("cookie","sugar","tea"),Array(6,5,3),(a: String,b: Int) => a.length
  //    .equals(b)` works because types are provided in the function literal
  //
  ...
  //
  //        - Currying is needed so that syntax is the same as in if statement:
